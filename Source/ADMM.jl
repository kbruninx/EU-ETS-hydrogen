    ...
    for iter in iterations
        if convergence == 0
            ...
            @timeit TO "Update prices" begin
                ...
                push!(results[ "λ"]["H2"], [maximum([0,results[ "λ"]["H2"][end][jy] - ADMM["ρ"]["H2"][end]/(10*data["General"]["nReprDays"])*data["H2"]["conv_factor"]*ADMM["Imbalances"]["H2"][end][jy]]) for jy in mdict[agents[:all][end]].ext[:sets][:JY]])
                push!(results[ "λ"]["H2CN_prod"], results[ "λ"]["H2CN_prod"][end] - ADMM["ρ"]["H2CN_prod"][end]/(10*data["General"]["nReprDays"])*data["H2"]["conv_factor"]*ADMM["Imbalances"]["H2CN_prod"][end])
