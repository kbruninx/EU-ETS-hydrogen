...
function save_results(mdict::Dict,EOM::Dict,ETS::Dict,H2::Dict,ADMM::Dict,results::Dict,data::Dict,agents::Dict,sens) 
    ...
    vector_output = [data["scen_number"]; sens; ADMM["n_iter"]; ADMM["walltime"];ADMM["Residuals"]["Primal"]["ETS"][end];ADMM["Residuals"]["Primal"]["MSR"][end]; 
                     ADMM["Residuals"]["Primal"]["EOM"][end];ADMM["Residuals"]["Primal"]["REC_y"][end]+ADMM["Residuals"]["Primal"]["REC_m"][end]+ADMM["Residuals"]["Primal"]["REC_d"][end]+ADMM["Residuals"]["Primal"]["REC_h"][end]; ADMM["Residuals"]["Primal"]["H2_h"][end]+ADMM["Residuals"]["Primal"]["H2_d"][end]+ADMM["Residuals"]["Primal"]["H2_m"][end]+ADMM["Residuals"]["Primal"]["H2_y"][end]; ADMM["Residuals"]["Primal"]["H2CN_prod"][end]; ADMM["Residuals"]["Primal"]["H2CN_cap"][end]; ADMM["Residuals"]["Dual"]["ETS"][end]; ADMM["Residuals"]["Dual"]["EOM"][end]; ADMM["Residuals"]["Dual"]["REC_y"][end]+ADMM["Residuals"]["Dual"]["REC_m"][end]+ADMM["Residuals"]["Dual"]["REC_d"][end]+ADMM["Residuals"]["Dual"]["REC_h"][end]; ADMM["Residuals"]["Dual"]["H2_h"][end]+ADMM["Residuals"]["Dual"]["H2_d"][end]+ADMM["Residuals"]["Dual"]["H2_m"][end]+ADMM["Residuals"]["Dual"]["H2_y"][end];ADMM["Residuals"]["Dual"]["H2CN_prod"][end]; ADMM["Residuals"]["Dual"]["H2CN_cap"][end]; mdict["Industry"].ext[:parameters][:β]; results[ "λ"]["EUA"][end][2]; tot_em; tot_cost]
    ...
    mat_output = [Years ETS["CAP"] ETS["S"] sum(ETS["C"][:,:],dims=2) ETS["MSR"][2:end,12] ETS["TNAC"][3:end] sum(results["e"][m][end] for m in agents[:cc]) sum(results["e"][m][end] for m in agents[:ind]) sum(results["e"][m][end] for m in setdiff(agents[:ets],union(agents[:ind],agents[:ps],agents[:cc]))) sum(results["e"][m][end] for m in setdiff(agents[:ets],union(agents[:ind],agents[:h2s],agents[:cc]))) results[ "λ"]["EUA"][end] sum(results["b"][m][end] for m in agents[:cc]) sum(results["b"][m][end] for m in agents[:ind]) sum(results["b"][m][end] for m in setdiff(agents[:ets],union(agents[:ind],agents[:ps],agents[:cc]))) sum(results["b"][m][end] for m in setdiff(agents[:ets],union(agents[:ind],agents[:h2s],agents[:cc])))]
    CSV.write(joinpath(home_dir,string("Results_",data["nReprDays"],"_repr_days"),string("Scenario_",data["scen_number"],"_ETS_",sens,".csv")), DataFrame(mat_output,:auto), delim=";",header=["Year";"CAP";"Supply";"Cancellation";"MSR";"TNAC";"Emissions_CC";"Emissions_Ind";"Emissions_H2S"; "Emissions_PS"; "EUAprice";  "EUAs_CC"; "EUAs_Ind"; "EUAs_H2S"; "EUAs_PS"]);
    ...
    gw = Dict()
    for m in agents[:eom] # including electrolysis and dac demand 
    ...
    cc_cap = zeros(length(agents[:cc]),data["nyears"])
    cc_prod = zeros(length(agents[:cc]),data["nyears"])
    cc_add_cap = zeros(length(agents[:cc]),data["nyears"])

    mm = 1
    for m in agents[:cc]
        CAP_LT = mdict[m].ext[:parameters][:CAP_LT]
        LEG_CAP = mdict[m].ext[:parameters][:LEG_CAP]
        cc_add_cap[mm,:] = cap = value.(mdict[m].ext[:variables][:cap])
        cc_cap[mm,:] = [sum(CAP_LT[y2,jy]*cap[y2] for y2=1:jy) + LEG_CAP[jy] for jy in mdict[m].ext[:sets][:JY]]    
        cc_prod[mm,:] = -value.(mdict[m].ext[:expressions][:e]) 
        mm = mm+1
    end
    mat_output = [Years transpose(cc_add_cap) transpose(cc_cap) transpose(cc_prod)]
    CSV.write(joinpath(home_dir,string("Results_",data["nReprDays"],"_repr_days"),string("Scenario_",data["scen_number"],"_CC_",sens,".csv")), DataFrame(mat_output,:auto), delim=";",header=["Year";string.("ADD_CAP_",agents[:cc]);string.("CAP_",agents[:cc]);string.("CO2_captured_",agents[:cc])]);
