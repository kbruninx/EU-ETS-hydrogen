function update_ind_emissions!(mod::Model,data::Dict,ETS::Dict)
    ...
    for y = 4:data["nyears"] # Emissions are only price-dependent as of 2022 (y=4)
        λ_nom = maximum([0,mod.ext[:parameters][:λ_EUA][y]/(1+data["inflation"])^(y-3)]) # M€/MtCO2, discounted to 2021 values, limited to positive values
        ...
        if mod.ext[:parameters][:e][y-1] > data["max_em_change"]*mod.ext[:parameters][:e][3]/100
            if mod.ext[:parameters][:e][y-1] - mod.ext[:parameters][:e][y] > data["max_em_change"]*mod.ext[:parameters][:e][3]/100
                mod.ext[:parameters][:e][y] = mod.ext[:parameters][:e][y-1]-data["max_em_change"]*mod.ext[:parameters][:e][3]/100
            end
        end
